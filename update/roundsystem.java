...
public class RoundManager {
    ...
    private int calculateZombieCount(int r) {
        // Approximate classic counts, hard-capped for thermal headroom on mobile
        if (r == 1) return 6;
        if (r == 2) return 8;
        if (r == 3) return 10;
        if (r == 4) return 12;
        if (r == 5) return 14;
        int count = 18 + (r - 5) * 2;
        return Math.min(count, 24); // cap at $$24$$
    }

    private float getSpawnDelay(int r) {
        // Floor at $$0.25$$ s; never spawn faster than $$4$$ Hz to prevent overlap chaos
